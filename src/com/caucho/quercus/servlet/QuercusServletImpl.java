...
public class QuercusServletImpl extends HttpServlet
{
  ...
  protected Path getPath(QuercusHttpServletRequest req)
  {
    ...
    if (TRACE) {
        ...
        HttpServletRequest hreq = req.toRequest(HttpServletRequest.class);
        System.out.println("scriptPath.dispatcher : " + hreq.getRequestDispatcher(scriptPath));
        System.out.println("scriptPath.realPath   : " + hreq.getServletContext().getRealPath(scriptPath));
        System.out.println("path                  : " + path);
    }

    // GRIMBO - The new logic is here.  We ask the servlet container if there
    // is a real path for the requested resource.  If there is, we use it.
    // This lets us use Tomcat aliases/extraResourcePaths, and means all our
    // (PHP) resources do NOT have to live inside the webapp directory
    // (meaning we don't have to copy them into the webapp to test, etc).
    if (!path.isFile()) {
        HttpServletRequest hreq = req.toRequest(HttpServletRequest.class);
        Path realPath = new FilePath(hreq.getServletContext().getRealPath(scriptPath));
        if (TRACE) {
            System.out.println("realPath              : " + realPath);
            System.out.println("realPath.isFile       : " + realPath.isFile());
        }
        if (realPath.isFile()) {
            return realPath;
        }
